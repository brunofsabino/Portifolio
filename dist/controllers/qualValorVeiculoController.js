...
const procuraCar = (req, response) => {
    ...
    if (dados) {
        ...
        (() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const listIcarros = [''];
            let pup = puppeteer_1.default;
            const browser = yield pup.launch({ headless: false });
            console.log('teste1');
            const page = yield browser.newPage();
            yield page.goto(url, { timeout: 0 });
            console.log('teste2');
            ...
            if (dados === 'Consulta de Carros e utilitários pequenos') {
                ...
                if (anoVeiculo) {
                    ...
                    if (number2 === -1) {
                        ...
                        dataConsulta = arrayValues[13];
                        precoMedio = arrayValues[15];
                        yield page.goto(mercadoLivre, { timeout: 0 });
                        yield page.waitForSelector('.nav-search-input');
                        yield page.type('.nav-search-input', `${modelo}`);
                        yield page.keyboard.press('Enter');
                        yield page.waitForSelector(`[aria-label="${anoModelo}"]`);
                        yield page.click(`[aria-label="${anoModelo}"]`);
